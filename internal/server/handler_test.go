...
func TestHandler_handleStickySession(t *testing.T) {
	tests := []struct {
		...
		{
			name:            "Sticky enabled, no cookie set",
			stickyEnabled:   true,
			requestPath:     "/any/path",
			setEnvVars:      true,
			expectHandled:   false,
		...
		{
			name:            "Sticky enabled, path matches",
			stickyEnabled:   true,
			paths:           []string{"/app/*"},
		...
		{
			name:          "Sticky enabled, path doesn't match",
			stickyEnabled: true,
			paths:         []string{"/app/*"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectCookieSet {
				...
				if !found {
					t.Error("Expected sticky session cookie to be set, but it wasn't")
				}
			}
		})
	}
