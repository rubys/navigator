...
func TestWebSocketProxy_StripPath(t *testing.T) {
	var receivedPath string

	// Create WebSocket backend server that records the path
	upgrader := websocket.Upgrader{CheckOrigin: func(r *http.Request) bool { return true }}
	backend := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		receivedPath = r.URL.Path
		conn, err := upgrader.Upgrade(w, r, nil)
	...
	// Convert HTTP URL to WebSocket URL - no path component
	backendURL, _ := url.Parse(backend.URL)
	backendURL.Scheme = "ws"
	...
	// Convert to WebSocket URL with the full path including prefix
	wsURL := "ws" + strings.TrimPrefix(server.URL, "http") + "/showcase/regions/iad/cable"
	...
	if receivedPath != "/cable" {
		t.Errorf("Expected backend to receive '/cable', got '%s'", receivedPath)
	}
}
...
func TestWebSocketProxy_ProtocolNegotiation(t *testing.T) {
	// Track what headers the backend receives
	var backendReceivedHeaders http.Header
	selectedProtocol := "actioncable-v1-json"

	// Create WebSocket backend that verifies headers and selects protocol
	upgrader := websocket.Upgrader{
		CheckOrigin:  func(r *http.Request) bool { return true },
		Subprotocols: []string{selectedProtocol, "actioncable-unsupported"},
	}

	backend := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// Capture headers received by backend
		backendReceivedHeaders = r.Header.Clone()

		// Note: Backend WILL receive handshake headers (Key, Version, etc.)
		// but they are NEW headers generated by Navigator's WebSocket client
		// for the Navigator→Backend connection, not the client's original headers.
		// This is correct behavior - we have two separate WebSocket connections.

		// Verify Sec-WebSocket-Protocol IS forwarded (application-level)
		if r.Header.Get("Sec-WebSocket-Protocol") == "" {
			t.Error("Backend SHOULD receive Sec-WebSocket-Protocol (application-level)")
		}

		// Verify the protocol value matches what the client requested
		expectedProtocols := "actioncable-v1-json, actioncable-unsupported"
		if r.Header.Get("Sec-WebSocket-Protocol") != expectedProtocols {
			t.Errorf("Expected protocol header '%s', got '%s'",
				expectedProtocols, r.Header.Get("Sec-WebSocket-Protocol"))
		}
	...
	// Connect as WebSocket client with protocol negotiation
	clientHeader := http.Header{}
	clientHeader.Set("Sec-WebSocket-Protocol", "actioncable-v1-json, actioncable-unsupported")

	conn, resp, err := websocket.DefaultDialer.Dial(wsURL, clientHeader)
	if err != nil {
		t.Fatalf("WebSocket connection failed: %v", err)
	}
	defer conn.Close()
	...
	if resp.Header.Get("Sec-WebSocket-Protocol") != selectedProtocol {
		t.Errorf("Expected protocol %s, got %s", selectedProtocol, resp.Header.Get("Sec-WebSocket-Protocol"))
	}

	// Verify backend received the protocol header
	if backendReceivedHeaders.Get("Sec-WebSocket-Protocol") == "" {
		t.Error("Backend did not receive Sec-WebSocket-Protocol header")
	}

	// Test message exchange works
	_, message, err := conn.ReadMessage()
	if err != nil {
		t.Fatalf("Failed to read welcome message: %v", err)
	}

	if string(message) != `{"type":"welcome"}` {
	...
	if string(echo) != testMsg {
		t.Errorf("Expected echo %s, got %s", testMsg, string(echo))
	}
}
